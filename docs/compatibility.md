...
Root `actions/setup-node`, `actions/setup-java`, `actions/setup-python`, `actions/setup-go`, and `actions/setup-dotnet` invocations receive fresh job-bound cache-v2 credentials for every lifecycle phase, regardless of their inputs. If credentials or redaction are unavailable, the runtime emits a warning and runs the phase without cache credentials. Direct `actions/cache`, `restore`, and `save` invocations fail when credentials are unavailable.

These setup actions use a subprocess-scoped synthetic `GITHUB_SERVER_URL` when the real host would make their clients select cache v1. Each allowlist entry requires an audit of the action source and bundled dependencies to confirm that `GITHUB_SERVER_URL` affects only caching behavior and is not load-bearing for any request the action makes. The workflow expression context retains the real server URL. Other JavaScript and Docker actions, ordinary `run` steps, and native action adapters do not receive cache credentials.
