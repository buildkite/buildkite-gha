...
Reusable-only `workflow_call` files remain available to local callers but do not create groups. Selecting only reusable workflows is an error. Every applicable workflow is compiled. A workflow with safe compilation errors produces one failing child step instead of a skipped group; the step prints all redacted diagnostics and exits with status 1. Other workflows continue compiling and successful workflows retain their normal jobs. Parse, trigger, event-input, admission, artifact, and upload failures still abort the whole transaction; no partial pipeline is uploaded.
