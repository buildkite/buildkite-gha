...
`paths` and `paths-ignore` are unsupported because Buildkite `if_changed` has different semantics. Exact support requires GitHub's changed-file set: a two-dot diff for an existing-branch push, GitHub's special two-dot base for a new branch, or a three-dot pull-request diff. It must preserve deleted and renamed paths, ordered negative patterns, GitHub's diff limits, and its run-on-timeout or more-than-1,000-commits behavior.

The webhook payload is not a sufficient source. Push commit arrays can be truncated and are not the final two-dot diff, while reduced Buildkite snapshots contain no comparison base. The uploader also has no admission client or credential for GitHub's compare or pull-request-files APIs; job-scoped workflow tokens are issued later and cannot authorize admission. Private repositories and fork pull requests therefore need a separate, provenance-checked read-only service for the base event repository. Until that service supplies a complete result bound to the event repository, event kind, immutable head, comparison base or pull-request number, and GitHub limit outcome, path filters fail closed. Unsupported trigger events, path filters, push type/workflow filters, pull request tag/workflow filters, inexact activity types, and invalid include/ignore combinations are not approximated; they replace the affected workflow with a failing step. Trigger shapes are validated even on a different event, but only the selected event contributes a group condition.
