...
Direct `actions/cache`, `restore`, and `save` invocations and root `actions/setup-node`, `actions/setup-java`, `actions/setup-python`, `actions/setup-go`, and `actions/setup-dotnet` invocations receive fresh job-bound cache-v2 credentials for every lifecycle phase, regardless of their inputs. If credentials or redaction are unavailable, the runtime emits a warning and runs the phase without cache credentials.
