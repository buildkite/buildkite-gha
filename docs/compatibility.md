...
Evaluation also fails closed for explicit event files, reduced Buildkite snapshots, Git-detected renamed or copied files, any diff combining additions and deletions, more than the importer's 300-file local bound, oversized or malformed Git output, backslash-containing paths, missing or shallow history, stale or mismatched event data, or invalid patterns. Rejecting combined additions and deletions covers renames that GitHub and local Git might classify differently. Public, private, and fork pull requests use the checkout's existing Git authorization without giving the importer another credential.
