...
supported. `secrets: inherit` is accepted when the called workflow declares no
required secrets. Remote calls, explicit secret mappings, required called-workflow
secrets, and call-level conditions are unsupported.
...
workflow secret mappings and environment secrets are also unavailable. Action
metadata defaults cannot add a secret to the plan; such defaults fail
