...
JavaScript and Docker actions with compatible bundled cache clients also receive job-bound cache-v2 credentials when the service is available. Cache credentials are required when a root `actions/setup-node`, `actions/setup-java`, or `actions/setup-python` invocation enables a cache selector, or when root `actions/setup-go` or `actions/setup-dotnet` enables its Boolean `cache` input. They are also required when an `actions/setup-node` version declares and enables `package-manager-cache`. Each setup lifecycle phase fails before execution if credentials or redaction are unavailable.
