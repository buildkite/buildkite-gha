...
Upload accepts `*`, one literal, directory, or tracked workflow glob, or two or more explicit workflow path operands. `*` maps to tracked `.github/workflows/*.{yml,yaml}` files. Explicit lists require regular, tracked `.yml` or `.yaml` files inside the repository and reject directories, missing or untracked paths, other extensions, paths outside the repository, symlinks, and independent glob expansion. Canonical repository-relative paths are deduplicated and sorted before stable workflow identities and namespaced job keys are assigned.
