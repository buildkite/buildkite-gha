...
The hosted profile also admits v4.3.0 at [`0057852bfaa89a56745cba8c7296529d2fc39830`](https://github.com/actions/cache/tree/0057852bfaa89a56745cba8c7296529d2fc39830) and the exact v5.0.3 and v5.1.0 commits for runtime-proof collection. They are not part of the public compatibility contract until that hosted proof is complete. Other v4 and v5 commits and unknown v6 commits remain unsupported.

The v4.3.0 action declares Node 20 and bundles `@actions/cache` 4.1.0. The runtime executes that declaration with managed Node 24. Its cache-v2 service selection, root restore/post-save lifecycle, separate `restore` and `save` entry points, and tar with zstd-or-gzip archive versioning match the supported service boundary. Unlike v5.1.0 and v6.1.0, v4.3.0 does not include their improved read-only-token save diagnostics.
