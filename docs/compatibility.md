...
Audited `actions/cache` subprocesses use a trusted tool `PATH` and strip `BASH_ENV`, `ENV`, and exported Bash functions (`BASH_FUNC_*`). GNU tar may invoke a shell for compression; a trusted `PATH` alone does not exclude shell startup files or exported functions. This filtering does not apply to other actions.
