...
| `release` | Pipeline Triggers support all seven GitHub release activities. Native Buildkite release builds require **Additional Webhooks** > **Releases** and **Code** trigger mode and deliver only `published`, `created`, and `released`; workflows that select other activities emit `W_NATIVE_RELEASE_ACTIVITIES_UNDELIVERED`. Bare declarations select all activities. Explicit `types` preserve exact selection: a scalar selects one activity and an empty list selects none. Malformed types, unknown activities, and branch, tag, path, and workflow filters are rejected. GitHub does not trigger `created`, `edited`, or `deleted` for draft releases, and Buildkite rejects those deliveries. Pipeline Triggers require the GitHub Code Access App to select the workflow at the immutable peeled tag commit. The ref is `refs/tags/<tag_name>`. The SHA is the server-resolved peeled commit, or the checked-out commit for the native compatibility fallback. Existing hosted release `GITHUB_TOKEN` policy is unchanged. |
