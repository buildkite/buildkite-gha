...
Reusable-only `workflow_call` files remain available to local callers but do not create groups. Selecting only reusable workflows is an error. Safe event-dependent validation errors produce one failing child step per diagnostic instead of a skipped group. Each step prints the redacted diagnostic and exits with status 1. Parse, trigger, event-input, action-resolution, admission, artifact, and upload failures still abort the whole transaction; no partial pipeline is uploaded.
