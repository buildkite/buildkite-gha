...
| Public GitHub actions | Supported subset | Sources are resolved to exact commits anonymously by default. Importers may set `BUILDKITE_GHA_GITHUB_TOKEN` solely for public remote action-source metadata, ref, commit, and tarball requests to the GitHub API; `GH_TOKEN` and `GITHUB_TOKEN` are not fallbacks. Repository visibility is checked independently and credentials are stripped before archive redirects. Complete trees are digest-verified, and JavaScript/composite/Dockerfile runtime rules still apply. Private actions and actions that require unavailable GitHub services remain unsupported. Operators must configure the credential only in the importer process environment, never at Buildkite pipeline or build scope, because buildkite-gha cannot undo Buildkite-level environment propagation to generated jobs. |
