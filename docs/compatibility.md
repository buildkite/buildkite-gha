...
Pull-request path filters require the reserved linked-webhook metadata and use its exact base and head commits in a local three-dot Git diff before pipeline upload. The webhook repository, pull-request number, base branch, base commit, and head commit must match the Buildkite build and the local `origin` base branch. Both commits and their merge base must already exist in the checkout. Filtering supports added, modified, deleted, and type-changed paths. It fails closed for explicit event files, reduced Buildkite snapshots, Git-detected renamed or copied files, more than 300 changed files, missing or shallow history, stale or mismatched event data, malformed Git output, or invalid patterns. This lets public, private, and fork pull requests use the checkout's existing Git authorization without giving the importer another credential.
