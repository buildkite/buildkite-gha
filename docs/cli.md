...
All selected directly runnable workflows are represented in one atomic pipeline upload. Each successfully compiled workflow becomes an aggregate group whose label is `:github: <workflow-name>` or, for an unnamed workflow, its canonical path. The group depends on the importer; child jobs do not repeat that dependency. A skipped workflow becomes one top-level skipped command step. One provider check is named `Buildkite / <workflow-name-or-path> (<effective-event>)`. GitHub events publish a GitHub check; Origin events publish an Origin check. A reusable-only `workflow_call` file may be selected so local callers can resolve it, but it does not create a group. An input set containing only reusable workflows is an error.
  ...
  - label: ":github: PB-2731 runner user proof"
    ...
    plugins:
      - github-actions#v0.9.3:
          ...
          source-ref: 1f40c890725ae21d2a523b031167c57141f85b4e
