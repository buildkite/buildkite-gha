...
Release ingestion also requires reserved linked-webhook metadata. It binds the webhook activity to `BUILDKITE_GITHUB_ACTION` and the release tag to both `BUILDKITE_TAG` and `BUILDKITE_BRANCH`. For pipelines connected to the GitHub Code Access App, Buildkite resolves lightweight and annotated tags to a peeled commit and stores its full SHA before creating the build. The plugin carries that SHA into each plan without local commit normalization. For other pipelines, it resolves Buildkite's symbolic `HEAD` from the checkout as a compatibility fallback. Environment fallback cannot invent a release event. Enable **Additional Webhooks** > **Releases** only with **Code** trigger mode.
...
| Ordinary workflow secrets | Static names are resolved with `buildkite-agent secret get` in the destination job. Local reusable-workflow calls can pass this request authority one hop with `secrets: inherit`; every nested edge must repeat it. The job's Buildkite identity and Secret access policies are the sole authorization boundary. Values are registered with Agent and local redaction before use. |
...
Ordinary workflow secrets are Buildkite destination-job secrets, not GitHub repository, environment, event, or fork-scoped secrets. Workflow syntax adds no Buildkite authority: arbitrary workflow code already runs with the destination job's identity and can call `buildkite-agent secret get`. For repository-local reusable workflows, `secrets: inherit` only lets the compiler place that callee job's statically referenced names in its plan. It is one hop, does not support aliases or remote calls, and cannot let action metadata invent requests. Restrict the destination identity with Buildkite Secret access policies, which remain authoritative. Plans and generated pipeline YAML contain secret names only, never values. `GITHUB_TOKEN` remains on its separate workflow-token boundary and is unaffected by inheritance.

Workflow token issuance requires an organization feature and a default-off pipeline setting. For `GITHUB_TOKEN`, Buildkite reads and enforces only the top-level repository permission policy from the requesting workflow at the build's immutable commit. Native release builds require the GitHub Code Access App so Buildkite can persist that immutable commit; the local `HEAD` compatibility fallback does not provide server provenance for token issuance. Omitted permissions resolve to exactly `contents: read`; write permissions require an explicit top-level map. Top-level `read-all` and `write-all` become explicit maps of the 13 supported repository scopes in plans and token requests. They exclude `id-token`, `models`, `repository-projects`, `code-quality`, `metadata`, and `vulnerability-alerts`; no alias crosses the wire. Server support for validating each immutable source policy must be deployed before the supporting client. Explicit empty top-level permissions, top-level scopes resolving only to `none`, and job-level aliases cannot receive a token. Job-level and called-workflow repository permission maps do not narrow or expand `GITHUB_TOKEN`; the separate OIDC credential boundary retains job-level and called-workflow `id-token` narrowing. Buildkite denies incomplete or cyclic trigger and rebuild provenance. Pull-request ancestry retains a `contents: read` ceiling, and merge-queue ancestry is denied.
