...
func (r Runner) runCheckoutTarget(ctx context.Context, processor *commandOutputProcessor, workspace string, commit string, inputs map[string]string, provider, repository, sha, refOutput, pinnedRef string, credentialed bool) (Result, error) {
	result := newResult()
	const adapter = "checkout adapter"
	...
	if pinnedRef != "" && pinnedRef != sha {
		if err := run(env, false, "update-ref", pinnedRef, sha); err != nil {
			return result, err
		}
	}
