...
func TestEmitActionRuntimeRequirement(t *testing.T) {
	pipeline := Pipeline{
		CompilerStep:       "importer",
		DistributionDigest: testDigest("distribution"),
		RuntimeCacheScope:  RuntimeCacheScope("buildkite/example", "refs/heads/main"),
	...
	if step.Env["BUILDKITE_GHA_RUNNER_TOOL_CACHE"] != RunnerToolCacheDir(pipeline.RuntimeCacheScope) {
...
func TestRuntimeCacheScopeIsDeterministicAndDoesNotExposeRef(t *testing.T) {
	main := RuntimeCacheScope("buildkite/example", "refs/heads/main")
	if main != RuntimeCacheScope("BUILDKITE/EXAMPLE", "refs/heads/main") {
		t.Fatalf("repository casing changed cache scope: %q", main)
	}
	if main == RuntimeCacheScope("buildkite/example", "refs/heads/feature/sensitive-name") {
		t.Fatal("different refs share a runtime cache")
	}
	if main == RuntimeCacheScope("buildkite/example", "refs/pull/42/merge") {
		t.Fatal("branch and pull request share a runtime cache")
	}
	if strings.Contains(main, "main") || !cacheScopePattern.MatchString(main) {
		t.Fatalf("runtime cache scope exposes ref or violates path syntax: %q", main)
	}
}
...
func TestEmitMiseCacheOnlyForActionJobs(t *testing.T) {
	...
	for _, step := range document.Steps {
		...
		if step.Key == "javascript" && step.Env["BUILDKITE_GHA_RUNNER_TOOL_CACHE"] != RunnerToolCacheDir(RuntimeCacheScope("buildkite/example", "refs/heads/main")) {
