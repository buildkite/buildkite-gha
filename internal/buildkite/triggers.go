...
func LiveEventPredicate(event string) string {
	...
	case "schedule":
		return "(" + predicate + " || (" + fallbackEvent + ` && build.pull_request.id == null && build.source == "schedule"))`
	case "merge_group", "release", "issues", "issue_comment", "pull_request_review", "pull_request_review_comment", "deployment", "deployment_status", "create", "delete", "label", "fork", "public", "gollum":
		return predicate
	default:
		return ""
	}
...
func translateTrigger(t workflow.Trigger, expressions TriggerConditionExpressions, snapshot TriggerEventSnapshot, selected bool) (condition string, contributes bool, err error) {
	...
	case "workflow_call":
		return "", false, nil
	case "deployment", "deployment_status", "create", "delete", "fork", "public", "gollum":
