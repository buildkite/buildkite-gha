...
func emitWorkflow(out *bytes.Buffer, pipeline Pipeline, workflow preparedWorkflow) error {
	if failure := workflow.Failure; failure != nil {
		...
		command := `printf '+++ \033[31mGitHub Actions workflow diagnostics\033[0m\n' && printf '\033[31m%s\033[0m\n' ` + shellQuote(failure.Message) + ` && printf '%s\n' '--- Create annotation' && printf '%s' ` + shellQuote(failure.Annotation) + ` | buildkite-agent annotate --scope=job --style=error && exit 1`
