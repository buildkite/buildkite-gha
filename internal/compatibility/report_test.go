...
func TestProcessingReportV3PreservesPerEventOutcomes(t *testing.T) {
	...
	if decoded.Schema != ProcessingSchemaV3 || decoded.Result != "incompatible" || decoded.Status != Failed || len(decoded.Evaluations) != 16 || decoded.Evaluations[0].Report.Result != "admitted" || decoded.Evaluations[1].Report.Result != "incompatible" {
		t.Fatalf("decoded report = %#v", decoded)
	}
