...
func TestProcessingReportV3PreservesPerEventOutcomes(t *testing.T) {
	...
	for _, event := range []string{"merge_group", "release", "deployment", "deployment_status", "create", "delete", "label", "issue_comment", "pull_request_review", "pull_request_review_comment", "workflow_dispatch", "schedule"} {
	...
	if decoded.Schema != ProcessingSchemaV3 || decoded.Result != "incompatible" || decoded.Status != Failed || len(decoded.Evaluations) != 15 || decoded.Evaluations[0].Report.Result != "admitted" || decoded.Evaluations[1].Report.Result != "incompatible" {
		t.Fatalf("decoded report = %#v", decoded)
	}
