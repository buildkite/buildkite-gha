...
func TestSourceBackedWorkspaceActionRequiresRemoteWorkflowProvenance(t *testing.T) {
	...
	build := func() Job {
		...
		setTestSteps(&job, []testStep{{ID: "remote", Kind: "uses", Uses: "owner/repository/root@v1", Action: &ActionSelector{Lock: "a-0000000000000001"}}})
		job.Actions = []ActionLock{
			{
	...
	topLevel := build()
	setTestSteps(&topLevel, []testStep{{ID: "local", Kind: "uses", Uses: "./checked-out/.github/actions/privacy", Action: &ActionSelector{Lock: "a-0000000000000002"}}})
	topLevel.Actions = []ActionLock{topLevel.Actions[1]}
	synchronizeExecutionProgram(&topLevel)
