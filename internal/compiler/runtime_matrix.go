...
type RuntimeContinuation struct {
	Descriptor RuntimeOutputDescriptor `json:"descriptor"`
...
func (c RuntimeContinuation) Roots() []RuntimeMatrixRoot {
...
func (continuation RuntimeContinuation) DependentInstances() int {
...
// its local action paths resolve against and the identity of the workflow it
// came from, which a `$/` self-repository reference resolves against. It
// exists only inside one compilation, to resolve the continuation's action
// locks.
type deferredAction struct {
	job, path, workspace, uses string
	// digest and remote identify the workflow source the way SourceDigest
	// and RemoteWorkflow identify an expanded instance's.
	digest              string
...
func deferredJobActions(sourced sourcedJob) []deferredAction {
	...
	for i, step := range sourced.Steps {
		...
		actions = append(actions, deferredAction{
			...
			digest: sourced.digest, remote: cloneRemoteWorkflowSource(sourced.remote),
...
// workflowSourceResolver resolves the containing repository and commit of the
// deferred job's workflow for `$/` action references, as
// workflowSourceResolver does for an expanded instance of the same job.
func (a deferredAction) workflowSourceResolver(options Options) func(context.Context) (*RemoteWorkflowSource, error) {
	return workflowSourceResolver(JobInstance{SourcePath: a.path, SourceDigest: a.digest, RemoteWorkflow: a.remote}, options)
}
...
func resolveContinuationActions(ctx context.Context, ir IR, options Options) (continuations []RuntimeContinuation, referencesVars bool, err error) {
...
func (c RuntimeContinuation) JobLabel(jobID string) string {
...
// RuntimeOutputDescriptor is a compile-time-only description of one validated
// deferred scheduling boundary. It is not an executable plan or upload authority.
type RuntimeOutputDescriptor struct {
...
func describeRuntimeMatrix(job workflow.Job, sourcePath, sourceDigest string, needs map[string]needBinding, jobs map[string]workflow.Job, matricesByJob map[string][]map[string]any) (RuntimeOutputDescriptor, bool, error) {
	if job.Matrix == nil {
		return RuntimeOutputDescriptor{}, false, nil
	}
	shape := ""
	...
	if shape == RuntimeMatrixShapeInclude && (len(job.Matrix.Rows) != 0 || len(job.Matrix.Include) != 0 || len(job.Matrix.Exclude) != 0 || job.Matrix.ExcludeExpression != nil) {
		return RuntimeOutputDescriptor{}, true, errors.New("runtime matrix include output must be the complete matrix definition")
	}
	...
	descriptor, err := describeRuntimeOutput(job.ID, sourcePath, sourceDigest, span, shape, reference, needs, jobs, matricesByJob)
	return descriptor, true, err
}

// describeRuntimeOutput binds a scheduling value to one producer instance.
func describeRuntimeOutput(job, sourcePath, sourceDigest string, span workflow.Span, shape string, reference expression.NeedOutputReference, needs map[string]needBinding, jobs map[string]workflow.Job, matricesByJob map[string][]map[string]any) (RuntimeOutputDescriptor, error) {
	binding, err := exactRuntimeMatrixNeed(needs, reference.Job)
	if err != nil {
		return RuntimeOutputDescriptor{}, err
	}
	producerID := ""
	...
	if binding.projectOutputs {
		...
		if err != nil {
			return RuntimeOutputDescriptor{}, fmt.Errorf("runtime matrix producer %q: %w", reference.Job, err)
		}
	} else {
		if len(binding.members) != 1 {
			return RuntimeOutputDescriptor{}, fmt.Errorf("runtime matrix producer %q must resolve to exactly one job", reference.Job)
		}
		producerID = binding.members[0]
	}
	producer, exists := jobs[producerID]
	if !exists {
		return RuntimeOutputDescriptor{}, fmt.Errorf("runtime matrix producer %q is unavailable after graph resolution", reference.Job)
	}
	output, err := exactRuntimeMatrixOutput(producer.Outputs, requestedOutput)
	if err != nil {
		return RuntimeOutputDescriptor{}, fmt.Errorf("runtime matrix producer %q: %w", producerID, err)
	}
	producerMatrices, exists := matricesByJob[producerID]
	if !exists || len(producerMatrices) != 1 {
		return RuntimeOutputDescriptor{}, fmt.Errorf("runtime matrix producer %q must have exactly one statically expanded instance", producerID)
	}
	...
	if shape == RuntimeRunsOnShape {
		descriptor.Schema = RuntimeRunsOnSchemaV1
		descriptor.MaxOutputBytes = MaxRuntimeRunsOnBytes
...
func (descriptor RuntimeOutputDescriptor) Validate() error {
	if descriptor.Schema != RuntimeMatrixSchemaV1 && descriptor.Schema != RuntimeRunsOnSchemaV1 {
	...
	if descriptor.Schema == RuntimeRunsOnSchemaV1 {
		if descriptor.Shape != RuntimeRunsOnShape || descriptor.MaxOutputBytes != MaxRuntimeRunsOnBytes || descriptor.MaxInstances != 1 || descriptor.MaxGraphJobs != MaxRuntimeMatrixGraphJobs {
			return errors.New("runtime runs-on v1 shape or limits do not match the immutable schema")
		}
	} else if descriptor.Shape != RuntimeMatrixShapeObject && descriptor.Shape != RuntimeMatrixShapeInclude {
	...
	if descriptor.Schema == RuntimeMatrixSchemaV1 && (descriptor.MaxOutputBytes != MaxRuntimeMatrixBytes || descriptor.MaxInstances != MaxRuntimeMatrixInstances || descriptor.MaxGraphJobs != MaxRuntimeMatrixGraphJobs) {
...
func EncodeRuntimeMatrixDescriptor(descriptor RuntimeOutputDescriptor) ([]byte, error) {
	if err := descriptor.Validate(); err != nil {
		return nil, err
	}
...
func DecodeRuntimeMatrixDescriptor(source []byte) (RuntimeOutputDescriptor, error) {
	if len(source) > MaxRuntimeMatrixDescriptorBytes {
		return RuntimeOutputDescriptor{}, fmt.Errorf("runtime matrix descriptor is %d bytes, maximum is %d", len(source), MaxRuntimeMatrixDescriptorBytes)
	}
	if !utf8.Valid(source) {
		return RuntimeOutputDescriptor{}, errors.New("runtime matrix descriptor is not valid UTF-8")
	}
	if err := rejectRuntimeMatrixDuplicateKeys(source); err != nil {
		return RuntimeOutputDescriptor{}, fmt.Errorf("decode runtime matrix descriptor: %w", err)
	}
...
func ExpandRuntimeMatrixOutput(descriptor RuntimeOutputDescriptor, source []byte, existingStepKeys []string) ([]map[string]any, error) {
	if err := descriptor.Validate(); err != nil {
		return nil, err
	}
	if descriptor.Schema != RuntimeMatrixSchemaV1 {
		return nil, errors.New("matrix expansion requires a runtime matrix descriptor")
	}
