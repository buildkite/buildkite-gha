...
func TestProgramOwnedSecretInventory(t *testing.T) {
	...
	secrets, token, err := requiredSecrets(lowerWorkflowProgram(instance), instance.secretAuthority, []string{"ACTION_DEFAULT"}, true)
	if err != nil {
		t.Fatal(err)
	}
	want := []string{"ACTION_DEFAULT", "CONDITION", "JOB_ENV", "JOB_OUTPUT", "RUN"}
	if !reflect.DeepEqual(secrets, want) || !token {
		t.Fatalf("requiredSecrets() = %#v, %v, want %#v, true", secrets, token, want)
	}
}
