...
Commands:
  ...
  continue  Expand a needs-derived matrix after its producer job ran
...
var commandUsage = map[string]string{
	...
	"continue":       "Usage: buildkite-gha continue --continuation-digest <digest> --continuation-producer <job>\n",
}
...
func writeCommandHelp(stdout io.Writer, command string) {
	...
	case "continue":
		_, _ = fmt.Fprint(stdout, "\nupload emits a continue step for every job whose matrix.include comes from a needs.<job>.outputs value. The step runs after the producer job, reads the continuation record that upload stored as an importer artifact, verifies the producer's published result, expands the matrix, recompiles the workflow from the checkout, checks that every job the initial upload created compiled identically, and uploads only the deferred jobs with deterministic step keys. A rerun after a successful upload is rejected by Buildkite for its duplicate step keys; continue then confirms that the existing steps carry the same plans and exits 0. Any other rejection, a retried producer, or a changed workflow fails the step; retry the whole build.\n")
	case "upload":
		_, _ = fmt.Fprint(stdout, "\nEvery workflow operand must be an explicit .yml or .yaml path; use -- before paths that begin with a dash. Multiple operands must be tracked files inside the checked-out repository. Inputs are uploaded as one aggregate pipeline: successful workflows become groups, while failed or skipped workflows become top-level replacement steps. Reusable-only workflow_call files are imported through callers but do not become groups. Scheduled groups select the preserved GitHub event or, when it is absent, Buildkite's schedule source. Buildkite schedules retain cron ownership, so every scheduled workflow group is eligible on any Buildkite scheduled build. The importer asks the job-scoped Agent API to resolve runs-on selectors before using local runner mappings. Each repeatable --runner-queue argument maps one supported runs-on label to a fallback Buildkite queue. Runner labels are case-insensitive. Every supported Linux label defaults to the matching immutable hosted-toolchains image; --runner-image overrides it for a configured profile. Duplicate or unsupported mappings fail, unmapped supported Linux labels retain default agent targeting with that image, macos-latest falls back to the hosted macos-medium queue, and macos-14 and macos-15 accept explicit fallback queues. Each repeatable --runtime-distribution argument binds linux/amd64 or darwin/arm64 to a verified executable. Upload importers may run on either platform; the matching runtime defaults to the importer executable, and workflows targeting the other platform require its distribution. --private-reusable-workflows enables importer-only Git fallback with the job's existing Git credentials; it does not enable private actions. Generated Linux jobs provision and use a non-root runner identity by default; --experimental-runner-user=false temporarily disables this behavior. The option does not affect macOS jobs. The deprecated --runtime-queue hosted option is accepted for plugin compatibility but does not select a queue. Event precedence is an explicit event file, Buildkite's reserved webhook metadata, then reduced-fidelity Buildkite environment compatibility data; every source remains unsigned. Verified checkout jobs automatically use Buildkite repository-provider Git credentials when the job enables them; the deprecated --private-checkout option is accepted as a no-op. For jobs that declare a GitHub environment, the importer resolves environment protection rules and environment secret names automatically through the job-scoped Agent API. Workflows that declare environments fail when that resolution is unavailable.\n")
	}
}
