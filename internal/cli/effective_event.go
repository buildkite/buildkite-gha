...
func triggerFailureProcessingReport(input workflowInput, err error) compatibility.ProcessingReport {
	...
	if errors.As(err, &pathFilters) {
		...
		err = &compiler.ProcessingFinding{
			Stage: processing.StagePipeline, Code: processing.CodePipelineGeneration, Category: "compatibility",
			Path: input.Path, Line: 1, Column: 1,
			Message: message,
			Detail:  pathFilters.Error(), Err: err,
		}
	}
	report.AddFailure(input.Path, processing.StagePipeline, processing.CodePipelineGeneration, "compatibility", err)
...
func triggerProcessingReport(path string, source []byte) compatibility.ProcessingReport {
	...
	report.SetStage(processing.StageWorkflowParsing, compatibility.Passed)
	report.SetStage(processing.StageEventValidation, compatibility.Passed)
