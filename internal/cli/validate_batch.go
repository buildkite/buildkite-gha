...
func loadBatchValidationResult(path, workflow string) (compatibility.ProcessingReportV3, bool) {
	...
	for _, evaluation := range report.Evaluations {
		if seen[evaluation.Event] || evaluation.Source != "generated" || evaluation.Report.Schema != compatibility.ProcessingSchema ||
			(evaluation.Event != "create" && evaluation.Event != "delete" && evaluation.Event != "push" && evaluation.Event != "pull_request" && evaluation.Event != "merge_group" && evaluation.Event != "release" && evaluation.Event != "deployment" && evaluation.Event != "deployment_status" && evaluation.Event != "issues" && evaluation.Event != "issue_comment" && evaluation.Event != "pull_request_review" && evaluation.Event != "pull_request_review_comment" && evaluation.Event != "workflow_dispatch" && evaluation.Event != "schedule") {
			return compatibility.ProcessingReportV3{}, false
		}
