...
func (r continuationRun) checkDrift(bundle compiler.Bundle) error {
	...
	recorded := make(map[string]compiledJob, len(artifact.Graph))
	for _, job := range artifact.Graph {
	...
	for i, current := range compiledJobs(bundle) {
		if current.PlanDigest == "" {
			return fmt.Errorf("job %q has no plan after recompilation", current.Key)
		}
		if deferred[current.LogicalJob] {
			instance, jobPlan := bundle.IR.Jobs[i], plans[current.Key]
			if _, exists := recorded[current.Key]; exists {
				return fmt.Errorf("deferred job %q collides with a job the initial upload created", current.Key)
			}
			if _, uploaded := artifact.Runtimes[instance.Platform.String()]; !uploaded {
				return fmt.Errorf("deferred job %q runs on %s, but the importer uploaded no runtime for that platform; configure --runtime-distribution for it", current.Key, instance.Platform)
			}
			...
			if source, exists := artifact.Continuation.Sources[current.LogicalJob]; !exists || !source.Matches(instance) {
				return fmt.Errorf("deferred job %q comes from a different workflow source than the initial upload compiled; the workflow inputs changed", current.Key)
			}
			...
			for _, lock := range jobPlan.Job.Actions {
				if before, exists := recordedLocks[lock.ID]; !exists || !reflect.DeepEqual(before, lock) {
					return fmt.Errorf("deferred job %q uses action %s, which the initial upload did not resolve to that revision; the workflow inputs changed", current.Key, describeActionLock(lock))
				}
			}
			continue
		}
		...
		if !exists {
			return fmt.Errorf("job %q did not exist in the initial upload; the workflow inputs changed", current.Key)
		}
		if !before.equal(current) {
			return fmt.Errorf("job %q compiled differently from the initial upload; the workflow inputs changed", current.Key)
		}
		reproduced++
	}
