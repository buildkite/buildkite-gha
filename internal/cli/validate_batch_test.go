...
func TestValidateBatchWritesAndResumesAtomicReports(t *testing.T) {
	...
	for i, record := range records {
		workflow := []byte("on: [push, create, delete, label, " + events[i] + ", " + deployments[i] + "]\njobs:\n  test:\n    runs-on: ubuntu-latest\n    steps:\n      - run: echo ok\n")
	...
	for _, path := range reports {
		...
		if len(report.Evaluations) != 6 || report.Evaluations[0].Event != "push" || report.Evaluations[1].Event != wantDeployment || report.Evaluations[1].Report.Result != "admitted" || report.Evaluations[5].Event != wantEvent || report.Evaluations[5].Report.Result != "admitted" {
			t.Fatalf("report %q lost deployment or review evaluation: %#v", path, report.Evaluations)
		}
		for i, event := range []string{"create", "delete", "label"} {
