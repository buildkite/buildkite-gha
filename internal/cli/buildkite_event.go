...
func buildkiteEventSource(getenv func(string) string) ([]byte, error) {
	...
	if githubEvent != "" {
		...
		case "issues", "issue_comment", "pull_request_review", "pull_request_review_comment", "release", "merge_group", "deployment", "deployment_status", "create", "delete", "label", "fork", "public":
	...
	if event == "create" || event == "delete" || event == "label" || event == "fork" || event == "public" {
		...
		if event == "delete" || event == "label" || event == "fork" || event == "public" {
