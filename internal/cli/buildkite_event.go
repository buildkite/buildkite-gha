...
func buildkiteEventSource(getenv func(string) string) ([]byte, error) {
	...
	if githubEvent != "" {
		...
		case "issues", "issue_comment", "pull_request_review", "pull_request_review_comment", "release", "merge_group", "deployment", "deployment_status", "create", "delete", "label", "fork", "public", "gollum", "page_build", "watch":
	...
	if event == "watch" || event == "create" || event == "delete" || event == "label" || event == "fork" || event == "public" || event == "gollum" || event == "page_build" {
		...
		if event == "watch" || event == "delete" || event == "label" || event == "fork" || event == "public" || event == "gollum" || event == "page_build" {
...
func buildkiteWebhookEventSource(getenv func(string) string, webhook []byte) ([]byte, error) {
	...
	if (event == "label" || event == "watch") && payload["action"] != getenv("BUILDKITE_GITHUB_ACTION") {
