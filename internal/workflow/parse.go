...
func Parse(path string, source []byte) (*Workflow, error) {
	...
	for i := range owned.Triggers {
		...
		case "issues", "issue_comment", "label", "release", "pull_request_review", "pull_request_review_comment", "fork", "public", "gollum", "page_build", "watch", "milestone", "branch_protection_rule", "discussion", "discussion_comment":
