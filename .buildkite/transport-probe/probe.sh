...
readonly importer_key="transport-probe-importer"
readonly producer_key="transport-probe-producer"
readonly consumer_key="transport-probe-consumer"
readonly marker_prefix="buildkite-gha/v1/uploads/${importer_key}"
readonly probe_path=".buildkite/transport-probe/probe.sh"
...
sign_json() {
  require_env TRANSPORT_PROBE_SIGN_JSON
  "${TRANSPORT_PROBE_SIGN_JSON}" < "$1"
}

verify_json() {
  require_env TRANSPORT_PROBE_VERIFY_JSON
  "${TRANSPORT_PROBE_VERIFY_JSON}" < "$1"
}
...
write_claims() {
  ...
  jq -Scn \
    ...
    --arg queue "${TRANSPORT_PROBE_RUNTIME_QUEUE}" \
    --arg event_name "${TRANSPORT_PROBE_EVENT_NAME}" \
    --arg repository "${TRANSPORT_PROBE_REPOSITORY}" \
    --arg ref "${TRANSPORT_PROBE_REF}" \
    --arg commit "${TRANSPORT_PROBE_COMMIT}" \
    --arg event_digest "${TRANSPORT_PROBE_EVENT_DIGEST}" \
...
bootstrap() {
  require_env BUILDKITE_BUILD_ID BUILDKITE_ORGANIZATION_ID BUILDKITE_PIPELINE_ID TRANSPORT_PROBE_RUNTIME_QUEUE TRANSPORT_PROBE_EVENT_NAME TRANSPORT_PROBE_REPOSITORY TRANSPORT_PROBE_REF TRANSPORT_PROBE_COMMIT TRANSPORT_PROBE_EVENT_DIGEST
  require_name TRANSPORT_PROBE_RUNTIME_QUEUE "${TRANSPORT_PROBE_RUNTIME_QUEUE}"
  require_digest TRANSPORT_PROBE_EVENT_DIGEST "${TRANSPORT_PROBE_EVENT_DIGEST}"
  ...
  require_commit TRANSPORT_PROBE_COMMIT "${TRANSPORT_PROBE_COMMIT}"
  local work
  work="$(mktemp -d)"
  cleanup_path="${work}"

  mkdir -p "${work}/buildkite-gha/v1/plans/${producer_key}" "${work}/buildkite-gha/v1/plans/${consumer_key}"
  printf '%s' '{"logical_job":"producer","required_capabilities":["network"],"schema":"transport-probe-plan/v1"}' > "${work}/producer.plan.json"
  printf '%s' '{"logical_job":"consumer","needs":["producer"],"required_capabilities":[],"schema":"transport-probe-plan/v1"}' > "${work}/consumer.plan.json"
  ...
  if [[ "${TRANSPORT_PROBE_TAMPER_BINDING:-}" == "1" ]]; then
  ...
  cat > "${pipeline}" <<YAML
steps:
  - label: ":test_tube: Transport probe producer"
    key: "${producer_key}"
    command: "${probe_path} producer"
    agents:
      queue: "${TRANSPORT_PROBE_RUNTIME_QUEUE}"
    env:
      TRANSPORT_PROBE_PLAN_DIGEST: "${producer_digest}"
      TRANSPORT_PROBE_PLAN_PRODUCER: "${importer_key}"
      TRANSPORT_PROBE_BINDING_JTI: "${producer_jti}"
      TRANSPORT_PROBE_VERIFY_JSON: "scripts/transport-probe-verify-json"
    plugins:
    ...
    depends_on:
      - step: "${importer_key}"
        allow_failure: false
  - label: ":test_tube: Transport probe consumer"
    key: "${consumer_key}"
    command: "${probe_path} consumer"
    agents:
      queue: "${TRANSPORT_PROBE_RUNTIME_QUEUE}"
    env:
      TRANSPORT_PROBE_PLAN_DIGEST: "${consumer_digest}"
      TRANSPORT_PROBE_PLAN_PRODUCER: "${importer_key}"
      TRANSPORT_PROBE_BINDING_JTI: "${consumer_jti}"
      TRANSPORT_PROBE_PRODUCER_PLAN_DIGEST: "${producer_digest}"
      TRANSPORT_PROBE_VERIFY_JSON: "scripts/transport-probe-verify-json"
    plugins:
      - mise#a5845c5082d3a4fe36dd77ae74973dfc86fc91a2:
          version: "2026.5.12"
...
YAML
  ...
  jq -Scn \
    ...
    '{phase:"expected",intent:{build_id:$build_id,importer_key:$importer_key,pipeline_digest:$pipeline_digest,jobs:[{key:"transport-probe-consumer",plan_digest:$consumer_digest},{key:"transport-probe-producer",plan_digest:$producer_digest}]}}' > "${work}/expected.json"
  ...
  if [[ -n "${existing}" ]]; then
    ...
    if [[ "${TRANSPORT_PROBE_INTERRUPT_AFTER_UPLOAD:-}" == "1" ]]; then
...
load_and_verify_plan() {
  ...
  require_env BUILDKITE_BUILD_ID BUILDKITE_ORGANIZATION_ID BUILDKITE_PIPELINE_ID BUILDKITE_STEP_KEY BUILDKITE_AGENT_META_DATA_QUEUE TRANSPORT_PROBE_PLAN_DIGEST TRANSPORT_PROBE_PLAN_PRODUCER TRANSPORT_PROBE_BINDING_JTI TRANSPORT_PROBE_EVENT_NAME TRANSPORT_PROBE_REPOSITORY TRANSPORT_PROBE_REF TRANSPORT_PROBE_COMMIT TRANSPORT_PROBE_EVENT_DIGEST TRANSPORT_PROBE_LOCAL_CAPABILITIES
  ...
  require_digest TRANSPORT_PROBE_PLAN_DIGEST "${TRANSPORT_PROBE_PLAN_DIGEST}"
  require_digest TRANSPORT_PROBE_EVENT_DIGEST "${TRANSPORT_PROBE_EVENT_DIGEST}"
  require_uuid BUILDKITE_BUILD_ID "${BUILDKITE_BUILD_ID}"
  require_uuid BUILDKITE_ORGANIZATION_ID "${BUILDKITE_ORGANIZATION_ID}"
  require_uuid BUILDKITE_PIPELINE_ID "${BUILDKITE_PIPELINE_ID}"
  require_uuid TRANSPORT_PROBE_BINDING_JTI "${TRANSPORT_PROBE_BINDING_JTI}"
  require_commit TRANSPORT_PROBE_COMMIT "${TRANSPORT_PROBE_COMMIT}"
  [[ "${TRANSPORT_PROBE_PLAN_PRODUCER}" == "${importer_key}" ]]
  jq -e 'type == "array" and . == (sort | unique) and all(.[]; . == "network" or . == "docker" or . == "privileged-container" or . == "secrets" or . == "provider-token-read" or . == "provider-token-write")' <<<"${TRANSPORT_PROBE_LOCAL_CAPABILITIES}" >/dev/null
  local prefix="buildkite-gha/v1/plans/${step_key}/${TRANSPORT_PROBE_PLAN_DIGEST#sha256:}"
  buildkite-agent artifact download "${prefix}/*" "${work}" --step "${TRANSPORT_PROBE_PLAN_PRODUCER}"
  [[ "$(sha256_file "${work}/${prefix}/plan.json")" == "${TRANSPORT_PROBE_PLAN_DIGEST}" ]]
  ...
  jq -e \
    ...
    --arg commit "${TRANSPORT_PROBE_COMMIT}" \
    --arg event_digest "${TRANSPORT_PROBE_EVENT_DIGEST}" \
    --arg plan_digest "${TRANSPORT_PROBE_PLAN_DIGEST}" \
    --arg iss "${binding_issuer}" \
    --arg jti "${TRANSPORT_PROBE_BINDING_JTI}" \
    --argjson now "${now}" \
  ...
  jq -e --slurpfile claims "${work}/claims.json" --argjson local "${TRANSPORT_PROBE_LOCAL_CAPABILITIES}" \
...
producer() {
  ...
  require_env TRANSPORT_PROBE_REDACTION_SECRET
  printf 'agent redaction canary: %s\n' "${TRANSPORT_PROBE_REDACTION_SECRET}"
  mkdir -p "${work}/buildkite-gha/v1/results/${producer_key}"
  local result="success"
  [[ "${TRANSPORT_PROBE_PRODUCER_FAIL:-}" == "1" ]] && result="failure"
  jq -Scn --arg build_id "${BUILDKITE_BUILD_ID}" --arg job_id "${BUILDKITE_JOB_ID}" --arg step_key "${producer_key}" --arg plan_digest "${TRANSPORT_PROBE_PLAN_DIGEST}" --arg result "${result}" \
    '{schema:"buildkite-gha/result-manifest/v2",plan_digest:$plan_digest,producer:{build_id:$build_id,job_id:$job_id,step_key:$step_key},result:$result,outputs:[{name:"message",value:"transport-probe-producer"}],artifacts:[]}' \
...
consumer() {
  ...
  require_env TRANSPORT_PROBE_PRODUCER_PLAN_DIGEST
  require_digest TRANSPORT_PROBE_PRODUCER_PLAN_DIGEST "${TRANSPORT_PROBE_PRODUCER_PLAN_DIGEST}"
  ...
  [[ "${TRANSPORT_PROBE_PRODUCER_FAIL:-}" == "1" ]] && expected_result="failure"
  jq -e --arg build_id "${BUILDKITE_BUILD_ID}" --arg job_id "${producer_job_id}" --arg step_key "${producer_key}" --arg plan_digest "${TRANSPORT_PROBE_PRODUCER_PLAN_DIGEST}" --arg result "${expected_result}" \
    '.schema == "buildkite-gha/result-manifest/v2" and .plan_digest == $plan_digest and .producer.build_id == $build_id and .producer.job_id == $job_id and .producer.step_key == $step_key and .result == $result and .outputs == [{"name":"message","value":"transport-probe-producer"}] and .artifacts == []' "${manifest}" >/dev/null
